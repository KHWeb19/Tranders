    ...
    <div>
        <div style="padding:40px">
            <v-row justify="center" style="padding-bottom: 20px">
                <v-card-title style="font-size: 30px;">구매 내역</v-card-title>
            </v-row>
            <section class="card-wrap">
                <v-col
                    class="none-product"
                    v-if="!buys || (Array.isArray(buys) && buys.length === 0)">
                    현재 구매 내역이 존재하지 않습니다!
                </v-col>
                <v-col v-else
                    v-for="productBoard in buys" :key="productBoard.productNo"
                    cols="4">
