<template>
  <div>
    <div style="padding-bottom: 20px" class="home_style" v-if="near === null">
    ...
    <div style="padding-bottom: 20px" class="home_style" v-else>
      ...
      <!--   정보가 없을 경우에     -->
      <div style="margin-top: 20px">
        <div style="height: auto; font-size: 23px; padding-top: 2px; padding-bottom: 2px">
          {{near.placeName}} 입니다.
        </div>
        ...
        <div style="padding-top: 30px">
          <span style="font-size: 25px;"><v-icon x-large>mdi-map-marker-outline</v-icon> {{ near.address }}</span>
        </div>
...
<!--          <span style="font-size: 25px;"><v-icon x-large>mdi-clock-time-five-outline</v-icon> {{boss.startTime}} ~ {{boss.endTime}}</span>-->
        ...
        <div style="padding-top: 15px">
          <span style="font-size: 25px;"><v-icon x-large>mdi-earth</v-icon> {{near.marketHomePage}}</span>
        </div>
      </div>
  ...
  methods: {
    ...
    initMap(){
      ...
      this.mapInstance = new kakao.maps.Map(container, {
        center: new kakao.maps.LatLng(this.near.lat, this.near.lng),
        level: 2,
      });

      this.markInstance = new kakao.maps.Marker({
        position: new kakao.maps.LatLng(this.near.lat, this.near.lng),
        //clickable: true
      })
